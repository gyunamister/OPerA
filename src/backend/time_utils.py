...
def get_event_timestamp(event):
    # TODO FIXXxx
    return make_timezone_aware(event[xes_constants.DEFAULT_TIMESTAMP_KEY])
...
def tests():
    ...
    assert add(pre_dst, timedelta(hours=3)) == datetime(
        2020, 10, 25, 3, 0, tzinfo=ber)
    assert next_day(pre_dst) == datetime(2020, 10, 26, tzinfo=ber)
    assert add_relative(pre_dst, relativedelta(days=1)) == datetime(
        2020, 10, 26, 1, 0, tzinfo=ber)
    assert add_relative(pre_dst, relativedelta(hours=24)) == datetime(
        2020, 10, 26, 0, 0, tzinfo=ber)
